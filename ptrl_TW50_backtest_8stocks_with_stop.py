# -*- coding: utf-8 -*-
"""
Pro Trader RL - 8 Stocks Backtest (Flexible Split Model Version)
Features:
- Uses models trained by `ptrl_TW50_split_train.py`
- Specific 8 Stocks Selection: 2330, 2317, 2454, 2382, 3231, 2357, 3017, 3661
- Max 5 Positions, 20% Capital Each
- Hard Stop Enabled (10% Loss) / No Time Stop
...
def fetch_tw50_data(data_path, start_date="2000-01-01"):
    # Modified to only include the specific 8 stocks + ^TWII for benchmark
    tickers = [
        "^TWII",
...
class DetailedBacktesterFlexible:
    def __init__(self, data_dict, buy_model, sell_model, start_date, end_date, initial_capital=1000000):
        ...
        self.inventory = {}
        # Modified: Max 5 positions, 20% each
    ...
    def run(self):
        ...
        for current_date in tqdm(self.dates):
            ...
            for ticker in list(self.inventory.keys()):
                ...
                # Modified: Hard Stop Enabled, Time Stop Disabled
    ...
    if not daily_stats.empty:
        ...
        for trade in trade_logs:
            ...
            if date in daily_stats.index:
                val = daily_stats.loc[date]['Total_Value']
                ticker = trade['Ticker']
                price = trade['Price']
                action = trade['Action']
                
                if action == 'Buy':
                    plt.annotate(f'Buy {ticker}\n@{price:.1f}', 
                                 xy=(date, val), 
                                 xytext=(date, val * 0.95),
                                 arrowprops=dict(facecolor='red', shrink=0.05),
                                 fontsize=8, color='red')
                elif action == 'Sell':
                    plt.annotate(f'Sell {ticker}\n@{price:.1f}', 
                                 xy=(date, val), 
                                 xytext=(date, val * 1.05),
                                 arrowprops=dict(facecolor='green', shrink=0.05),
                                 fontsize=8, color='green')

        plt.title(f'8 Stocks Backtest Equity Curve ({start_date} ~ {end_date})')
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(RESULTS_PATH, f"8stocks_backtest_result_{timestamp}.png"))
        print("Plot saved.")
        
        # Save Trade Logs
        if trade_logs:
            log_df = pd.DataFrame(trade_logs)
            log_path = os.path.join(RESULTS_PATH, f"8stocks_backtest_trades_{timestamp}.csv")
