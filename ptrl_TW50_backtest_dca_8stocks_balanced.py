# -*- coding: utf-8 -*-
"""
Pro Trader RL (DCA Backtest - 8 Stocks Balanced)
Features:
...
- Constraints: Max 8 positions, Max 12.5% per stock
"""
...
class DetailedBacktesterDCA:
    def __init__(self, data_dict, buy_model, sell_model, start_year):
        ...
        self.inventory = {}
        # Constraints: 8 Stocks, but mainly controlled by budget allocation now
        ...
        # DCA Settings
        self.dca_period = 20
        # 12.5% of yearly budget per stock => 1/8
        # Then split into 1/12 chunks per period
        self.amount_per_stock_per_period = (self.yearly_budget / 8) / 12
        
        self.last_dip_buy_date = {} # Track last dip buy per ticker
        self.period_quota_used = {t: False for t in self.tickers} # Independent quota per stock
        self.dca_counter = 0

    def run(self):
        print(f"執行回測 (Balanced 8-Stock DCA)...")
        print(f"Per Stock Per Period Amount: {self.amount_per_stock_per_period:.0f}")
        records = []
        ...
        for i, current_date in enumerate(tqdm(self.dates)):
            ...
            # 1. AI Dip Buy Check (Independent per stock)
            # Loop through ALL tickers. If condition met and quota not used, buy.
            
            if self.cash >= self.amount_per_stock_per_period:
                # Randomize order slightly to avoid bias if cash runs out (unlikely with this logic)
                check_tickers = list(self.data_dict.keys())
                np.random.shuffle(check_tickers)
                
                for ticker in check_tickers:
                    if self.period_quota_used.get(ticker, False):
                        continue # Already bought this period (either Dip or Regular shouldn't happen yet)
                    ...
                    if row['Close'] < row['MA60'] and days_since_dip > 20:
                        ...
                        if action == 1: # AI says Buy
                            shares = int(self.amount_per_stock_per_period / row['Close'])
                            ...
                            if self.cash >= cost:
                                ...
                                self.period_quota_used[ticker] = True

            # 2. Regular DCA
            # Check every 20 days. If a stock hasn't used quota, buy.
            if self.dca_counter % self.dca_period == 0:
                for ticker in self.data_dict.keys():
                    if not self.period_quota_used.get(ticker, False):
                        df = self.data_dict[ticker]
                        if current_date not in df.index: continue
                        row = df.loc[current_date]
                        ...
                        if self.cash >= cost:
                            ...
                            # Mark used just in case, though we reset immediately after this block conceptually
                            self.period_quota_used[ticker] = True

                # Reset quota for next period
                self.period_quota_used = {t: False for t in self.tickers}
            
            self.dca_counter += 1

            # --- Benchmark Strategy (Pure DCA balanced) ---
            # Buy 1/8 * 1/12 of budget for EVERY stock every 20 days
            # Assuming Benchmark follows same schedule
            if i % self.dca_period == 0:
                bench_amt = self.amount_per_stock_per_period
                for ticker in self.data_dict.keys():
                    df = self.data_dict[ticker]
                    if current_date not in df.index: continue
                    row = df.loc[current_date]
                    
                    shares = int(bench_amt / row['Close'])
                    if shares == 0: continue
                    cost = shares * row['Close'] * (1 + 0.001425)
    ...
    def export_results(self, daily_stats, results_path):
        ...
        if self.trade_logs:
            ...
            log_path = os.path.join(results_path, f"dca_8stocks_balanced_trade_log_{timestamp}.csv")
        ...
        plt.title(f"DCA Backtest (8 Stocks Balanced) - AI Dip Buy vs Regular DCA")
        plt.xlabel("Date")
        plt.ylabel("Portfolio Value (TWD)")
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(results_path, f"dca_8stocks_balanced_result_{timestamp}.png"))
        plt.close()
        ...
        if self.trade_logs:
            ...
            for ticker in self.data_dict.keys():
                ...
                plt.savefig(os.path.join(results_path, f"plot_dca_balanced_{safe_ticker}_{timestamp}.png"))
                plt.close()
