# -*- coding: utf-8 -*-
"""
Pro Trader RL (Split Training Version - MA Filter)
Features:
- Custom Train/Test split
- MA-based Signal Filtering:
  - Buy Filter: Close < MA60
  - Sell Filter: Close > MA10 * 1.10
- Unique Model Names: ppo_buy_ma, ppo_sell_ma
"""
...
class SellEnvPaper(gym.Env):
    def __init__(self, data_dict):
        ...
        for t, df in data_dict.items():
            ...
            # Include Sell Filter
            sell_filters = df['Signal_Sell_Filter'].values
            
            for idx in buy_indices:
                if idx + 120 < len(df):
                    episode_features = feature_data[idx : idx+120]
                    episode_prices = close_prices[idx : idx+120]
                    episode_sell_filters = sell_filters[idx : idx+120]
                    ...
                    self.episodes.append({
                        ...
                        'returns': returns,
                        'buy_price': buy_price,
                        'sell_filters': episode_sell_filters
                    })
    ...
    def step(self, action):
        current_return = self.current_episode['returns'][self.day]
        can_sell = self.current_episode['sell_filters'][self.day]
        
        reward = 0.0
        done = False
        is_profitable = (current_return >= 1.10)

        # If filter says NO, force Hold (action=0)
        if not can_sell:
            action = 0
...
if __name__ == "__main__":
    ...
    # 5. 訓練 Buy Agent (MA)
    print("\n=== 檢查 Buy Agent 模型 (MA Filter) ===")
    TOTAL_TIMESTEPS_BUY = 2_000_000
    TOTAL_TIMESTEPS_SELL = 1_000_000
    
    BUY_RESULTS_PATH = os.path.join(RESULTS_PATH, "buy_ma")
    SELL_RESULTS_PATH = os.path.join(RESULTS_PATH, "sell_ma")
    for p in [BUY_RESULTS_PATH, SELL_RESULTS_PATH]:
        if not os.path.exists(p): os.makedirs(p)

    checkpoint_callback = CheckpointCallback(save_freq=5000, save_path=MODELS_PATH, name_prefix="ppo_buy_ma")
    eval_callback = EvalCallback(eval_env, best_model_save_path=os.path.join(MODELS_PATH, "best_buy_ma"),
    ...
    if os.path.exists(final_buy_path) or os.path.exists(best_buy_path):
        ...
        if choice == '1':
            ...
            print("🔄 選擇接續訓練...")
            ckpt_files = glob.glob(os.path.join(MODELS_PATH, "ppo_buy_ma_*_steps.zip"))
            latest_ckpt = None
            if ckpt_files:
            ...
            ckpt_pattern = os.path.join(MODELS_PATH, "ppo_buy_ma_*_steps.zip")
    ...
    if should_train_buy:
        if buy_model is None:
            print("🚀 開始全新訓練 Buy Agent (MA Filter)...")
            buy_model = PPO("MlpPolicy", buy_env, **ppo_params)
        else:
            print("🚀 接續訓練 Buy Agent (MA Filter)...")
            
        buy_model.learn(total_timesteps=TOTAL_TIMESTEPS_BUY, callback=callbacks, reset_num_timesteps=False)
        buy_model.save(os.path.join(MODELS_PATH, "ppo_buy_ma_final"))
    
    # 6. 訓練 Sell Agent (MA)
    print("\n=== 檢查 Sell Agent 模型 (MA Filter) ===")
    sell_checkpoint_callback = CheckpointCallback(save_freq=5000, save_path=MODELS_PATH, name_prefix="ppo_sell_ma")
    sell_eval_callback = EvalCallback(eval_sell_env, best_model_save_path=os.path.join(MODELS_PATH, "best_sell_ma"),
    ...
    sell_ckpt_files = glob.glob(os.path.join(MODELS_PATH, "ppo_sell_ma_*_steps.zip"))
